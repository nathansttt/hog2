...
#ifndef FLRTAStarUnit_H
#define FLRTAStarUnit_H
...
class FLRTAStarUnit : public Unit<state, action, environment> {
	
public:
	FLRTAStarUnit(state &s, state &target, FLRTA::FLRTAStar<state, action, environment> *alg)
	{
		currentLoc = s;
		goalLoc = target;
		//goal = target;
	...
	void StartNewTrial(StatCollection *s) {
		...
		nodesExpanded = 0;
		nodesTouched = 0;
		//		printf("FLRTA*: New trial: %f learned this trial; %f total\n",
...
private:
	...
	FLRTA::FLRTAStar<state, action, environment> *algorithm;			// pointer to the LRTA* algorithm
...
template <class state, class action, class environment>
void FLRTAStarUnit<state, action, environment>::OpenGLDraw(const environment *e, const SimulationInfo<state,action,environment> *si) const
{
	...
	//	for (typename GtateStorage::const_iterator it = hashTable.begin(); it != hashTable.end(); it++)
	//	{
