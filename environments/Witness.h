...
public:
    ...
    enum controlState {
        kWaitingStart, kInPoint, kBetweenPoints, kWaitingRestart
    };
...
public:
    enum edgeConstraint {
        kNoConstraint = 0, kMustCross = 1, kCannotCross = 2, kEdgeConstraintCount = 3
    };

    enum legality {
        kLegal, kNotAtEnd, kNotAtStart, kNotValidAction, kHitCannotCross, kHitStart, kHitLine
    };
    ...
    // tetrisCount(0)
    {
        static_assert((width <= 8) && (height <= 8) && (width > 0) && (height > 0),
    ...
    void Reset() {
        ...
        for (int c = 0; c < kRegionConstraintCount; c++) constraintCount[c] = 0;
        constraintCount[kNone] = width * height;

        for (int x = 0; x < GetNumPathConstraints(); x++) pathConstraints[x] = kNoConstraint;
    ...
    void GetSuccessors(const WitnessState<width, height> &nodeID,
                       std::vector <WitnessState<width, height>> &neighbors) const;
    ...
    void DrawRegionConstraint(Graphics::Display &display, const WitnessRegionConstraint &constraint,
                              const Graphics::point &p3) const;
    ...
    const uint16_t tetrisBits[25] = {0, 0x8000, 0xC000, 0x8800, 0xC800, 0xC400, 0x8C00, 0x4C00, 0xE000,
                                     0x8880, 0xCC00, 0xE800, 0x8E00, 0xE200, 0x2E00, 0xC880, 0xC440, 0x88C0,
                                     0x44C0, 0xF000, 0x8888, 0xE400, 0x4E00, 0x8C80, 0x4C40};
    const uint64_t tetrisBits64[25] = {0,
                                       0x8000000000000000ull,
                                       0xC000000000000000ull,
                                       0x8080000000000000ull,
                                       0x80C0000000000000ull,
                                       0x40C0000000000000ull,
                                       0xC080000000000000ull,
                                       ...
                                       0xE020000000000000ull,
                                       0x8080C00000000000ull,
                                       0x4040C00000000000ull,
                                       0xC080800000000000ull,
    ...
    bool RecursivelyPlacePieces(int curr, uint64_t board, uint64_t oob, uint64_t posFootprint,
                                uint64_t negFootprint) const;

    void GetMouseActions(const WitnessState<width, height> &nodeID, std::vector <WitnessAction> &actions) const;

    void DoLine(Graphics::Display &display, const Graphics::point &p1, const Graphics::point &p2,
...
void Witness<width, height>::UndoAction(WitnessState<width, height> &s, WitnessAction a) const {
    ...
    if (s.path[len - 1].first <= width && s.path[len - 1].second <= height) {
        if (len > 1)
            s.UnoccupyEdge(s.path[len - 1].first, s.path[len - 1].second, s.path[len - 2].first,
                           s.path[len - 2].second);
...
double Witness<width, height>::HCost(const WitnessState<width, height> &node1,
                                     const WitnessState<width, height> &node2) const {
    return 0;
}
...
bool Witness<width, height>::GoalTest(const WitnessState<width, height> &node,
                                      const WitnessState<width, height> &goal) const {
    // Check constraints
...
template<int width, int height>
bool Witness<width, height>::RecursivelyPlacePieces(int curr, uint64_t board, uint64_t oob, uint64_t posFootprint,
                                                    uint64_t negFootprint) const {
...
template<int width, int height>
void Witness<width, height>::DrawRegionConstraint(Graphics::Display &display, const WitnessRegionConstraint &constraint,
                                                  const Graphics::point &p3) const {
    switch (constraint.t) {
        case kNone:
            break;
        ...
        case kStar: {
            display.FillNGon(p3, gapOffset / 5.0f, 4, 0, constraint.c);
            display.FillNGon(p3, gapOffset / 5.0f, 4, 45, constraint.c);
            break;
        }
        case kTetris:
        case kNegativeTetris: {
            int whichPiece = abs(constraint.parameter);
            bool negative = constraint.t == kNegativeTetris;  // constraint.parameter<0;
            if (whichPiece != 0) {
                float xOff = 0;
                float yOff = 0;
                switch (whichPiece) {
...
void Witness<width, height>::Draw(Graphics::Display &display) const {
    ...
    // start
    assert(start.size() > 0);
    for (const auto &i: start) display.FillCircle(GetScreenCoord(i.first, i.second), lineWidth * 3.f, lineColor);
